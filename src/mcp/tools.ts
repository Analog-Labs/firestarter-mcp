...
export function registerTools(server: McpServer, apiKey: string, apiBase: string) {
  ...
  server.tool(
    ...
    "Connect a seller's BigCommerce, Shopee, Lazada, Wix, or WooCommerce store to Firestarter. For Shopify use firestarter_connect_shopify instead (one-click install link); for TikTok Shop use firestarter_connect_tiktok. Call with just `platform` first to check for an existing connection or get platform-specific credential instructions; call again with credentials to connect.",
    {
      ...
      shop_domain: z.string().optional().describe("Store identifier: BigCommerce store hash, Shopee/Lazada shop id, Wix account id, or the WooCommerce store URL."),
    ...
    async ({ platform, access_token, shop_domain, consumer_key, consumer_secret }) => {
      ...
      try {
        ...
        const credentialInstructions: Record<string, string> = {
          ...
          woocommerce: "Go to your WordPress admin > WooCommerce > Settings > Advanced > REST API > 'Add Key', set permissions to 'Read'. Provide the Consumer Key and Consumer Secret separately (consumer_key/consumer_secret, not access_token) and your store URL as shop_domain.",
        };
        ...
        if (platform === "woocommerce") {
          ...
          await apiRequest("POST", "/v1/connections", { platform, access_token: encoded, shop_domain });
          return { content: [{ type: "text" as const, text: `**WooCommerce connected: ${shop_domain}**\nCatalog sync started - check results with firestarter_listings shortly.` }] };
        }
