...
export function registerTools(server: McpServer, apiKey: string, apiBase: string, onAuthError?: (apiKey: string) => void, opts?: RegisterToolsOptions) {
        ...
        if (!image_base64) {
          ...
          const attachNote = uploadRequest.verify_listing_id
            ...
            : uploadRequest.listing_id
              ? ` Every dropped file (photo or video) attaches to listing ${uploadRequest.listing_id} at full quality${uploadRequest.activate ? " and activation is requested automatically" : ""}.`
  ...
  server.tool(
    ...
    "Create a voucher (also called a coupon, promo code, or discount code) that buyers can apply to your listings. Requires a SELLER account with at least one listing — a community-market owner who only recommends other sellers' products has nothing of their own to discount; tiered access (firestarter_set_market_tiers) is the member-reward mechanism for that case. Supports percentage off, a fixed amount off, or free shipping, with an optional start/end date, usage cap, per-buyer limit, minimum order value, and scoping to a single listing. YOU FUND THE DISCOUNT: it comes out of your proceeds, and the platform fee is charged on the discounted total. Dates accept natural language — 'next Friday', 'in 2 weeks' — as well as ISO dates. The seller picks the code — ask for it before calling this, and never auto-generate one. A discount above 50% requires confirm_deep_discount — a flag attesting that the seller has confirmed that unusually deep number. If the voucher would leave the seller's cheapest orders below the payable minimum the call is rejected with an explanation rather than creating something whose orders would fail at payment.",
    {
      code: z.string().describe("The code buyers type, e.g. 'SUMMER20'. THE SELLER CHOOSES THIS — ask them for it and pass what they say. Do not invent one, and do not derive one from the discount (a seller who asked for '$2 off' has not asked for a code called SAVE2OFF); the code is what they will print, say out loud and put in a post, so it is theirs to pick. If they have no preference, offer a suggestion and use it only once they agree. 2-64 characters: letters, numbers, dashes or underscores. Case-insensitive; stored uppercase."),
